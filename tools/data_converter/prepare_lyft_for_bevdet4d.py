...
def obtain_sensor2top(lyft,
    ...
    Args:
        lyft (class): Dataset class in the nuScenes dataset.
...
def add_adj_info():
    ...
    for set in ['train', 'val']:
        lyft_version = 'v1.01-train'
        dataroot = './data/lyft/'

        dataset = pickle.load(open(f'{dataroot}lyft_infos_%s.pkl' % set, 'rb'))
        lyft = Lyft(data_path=os.path.join(dataroot, lyft_version),
                    json_path=os.path.join(dataroot, lyft_version, lyft_version),
                    verbose=True)
        ...
        for id in range(len(dataset['infos'])):
            ...
            pts_info = lyft.get('sample_data', sample['data']['LIDAR_TOP'])
            pts_dict = pts_info
            pts_dict['filename'] = os.path.join(lyft_version, pts_info['filename'])
            for adj in ['next', 'prev']:
                ...
                for cam in ['CAM_FRONT', 'CAM_FRONT_RIGHT', 'CAM_FRONT_LEFT',
                    ...
                    sample_data = lyft.get('sample_data', sample['data'][cam])
                    adj_list[cam] = []
                    count = 0
                    while count < max_adj:
                        if sample_data[adj] == '':
                            break
                        sd_adj = lyft.get('sample_data', sample_data[adj])
                        sample_data = sd_adj
                        adj_list[cam].append(dict(data_path=os.path.join(dataroot, lyft_version, sd_adj['filename']),
                ...
                for count in range(interval - 1, min(max_adj, len(adj_list['CAM_FRONT'])), interval):
                    ...
                    # get ego pose
                    pose_record = lyft.get('ego_pose', adj_list['CAM_FRONT'][count]['ego_pose_token'])
                    
                    cs_record = lyft.get('calibrated_sensor',
                                          pts_info['calibrated_sensor_token'])
                    pose_record = lyft.get('ego_pose', pts_info['ego_pose_token'])
                    ...
                    while len(sweeps_pts) < 10:
                        if not pts_info['prev'] == '':
                            pts_info = lyft.get('sample_data', pts_info['prev'])
                            try:
                                sweep = obtain_sensor2top(lyft, pts_info['prev'], l2e_t,
            ...
            previous_id = id
            if not sample['prev'] == '':
                sample_tmp = lyft.get('sample', sample['prev'])
                previous_id = map_token_to_id[sample_tmp['token']]
            next_id = id
            if not sample['next'] == '':
                sample_tmp = lyft.get('sample', sample['next'])
        ...
        with open(f'{dataroot}lyft_infos_%s_4d_interval%d_max%d.pkl' % (set, interval, max_adj), 'wb') as fid:
