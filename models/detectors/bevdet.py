...
class BEVDet(CenterPoint):
    def __init__(self, img_view_transformer=None, img_bev_encoder_backbone=None, img_bev_encoder_neck=None, 
                temporal=False, text_embeddings_path=None, visual=False, **kwargs):
    ...
    def build_features(self):
        ...
        self.vit.eval()
        self.vit.train = disabled_train
        for param in self.vit.parameters():
            param.requires_grad = False

        self.feat_embed = nn.Linear(1024, 768)
        
        if isinstance(self.feat_embed, nn.Conv2d):
            torch.nn.init.kaiming_normal_(self.feat_embed.weight)
        elif isinstance(self.feat_embed, nn.BatchNorm2d):
            self.feat_embed.weight.data.fill_(1)
            self.feat_embed.bias.data.zero_()
    ...
    def get_embed_features(self, input_img, input_x):
        feature_b, feature_n, feature_c, feature_h, feature_w = input_x.shape
        x = input_x.reshape(feature_b * feature_n, feature_c, feature_h, feature_w)
        feature_b, feature_c, feature_h, feature_w = x.shape
        times = math.ceil(feature_w/feature_h)
        intersection = math.ceil((feature_w-((times-1)*feature_h))/(times-1))
        cropped_feats = []
        for cropped_idx in range(times): cropped_feats.append(x[:,:,:,(feature_h-intersection)*cropped_idx:(feature_h-intersection)*cropped_idx+feature_h])

        cropped_embeds = []
        for cropped_idx in range(times):
            avg_feats = F.adaptive_avg_pool2d(cropped_feats[cropped_idx], (1, 1)).squeeze(2).squeeze(2)
            max_feats = F.max_pool2d(cropped_feats[cropped_idx], kernel_size=cropped_feats[cropped_idx].size()[2:]).squeeze(2).squeeze(2)
            feat_embed = self.feat_embed(torch.cat([avg_feats, max_feats], dim=-1))
            cropped_embeds.append(feat_embed)
        cropped_embeds = torch.cat(cropped_embeds, dim=0)

        with torch.no_grad():
            batch_size, num_cams, num_channels, imH, imW = input_img.shape
            img = input_img.reshape(batch_size * num_cams, num_channels, imH, imW)
            times = math.ceil(imW/imH)
            intersection = math.ceil((imW-((times-1)*imH))/(times-1))
            cropped_imgs = torch.cat([img[:,:,:,(imH-intersection)*times_idx:(imH-intersection)*times_idx+imH] for times_idx in range(times)], dim=0)
            cropped_imgs = torchvision.transforms.functional.resize(cropped_imgs, (224, 224))
            vit_result = self.vit.encode_image(cropped_imgs)
        return cropped_embeds, vit_result
    ...
    def forward_dummy(self, points=None, img_metas=None, img_inputs=None, **kwargs):
        img_feats, _, _ = self.extract_feat(points, img=img_inputs, img_metas=img_metas)
...
class CMKD_BEVDepth(BEVDepth):
    def __init__(self, img_view_transformer=None, 
                    ...
                    res_reg_loss_cfg=None,
                    **kwargs):
        super(CMKD_BEVDepth, self).__init__(**kwargs)
        if img_view_transformer:
            self.img_view_transformer = builder.build_neck(img_view_transformer)
        if img_bev_encoder_backbone:
            self.img_bev_encoder_backbone = builder.build_backbone(img_bev_encoder_backbone)
    ...
    def forward_train(self,
        ...
        bev_img, pts_feats, depth = self.extract_feat(
                                    points, img=img_inputs, img_metas=img_metas)
...
class BarlowTwinsLoss(nn.Module):
    def __init__(self, lambda_coeff=5e-3, feat_type='single', ver2=False):
        super().__init__()
        ...
        if not self.ver2:
            self.conv1 = torch.nn.Conv2d(256, 64, 1, 1, 0) # 64 x 128
            self.norm1 = torch.nn.BatchNorm2d(64)

            self.conv2 = torch.nn.Conv2d(64, 64, 3, 2, 1) # 64 x 64
            self.norm2 = torch.nn.BatchNorm2d(64)

            self.conv3 = torch.nn.Conv2d(64, 64, 3, 2, 1) # 64 x 32
            self.norm3 = torch.nn.BatchNorm2d(64)

            self.conv4 = torch.nn.Conv2d(64, 64, 3, 2, 1) # 64 x 16
            self.norm4 = torch.nn.BatchNorm2d(64)

            '''
            self.conv5 = torch.nn.Conv2d(64, 64, 3, 2, 1) # 64 x 8
            self.norm5 = torch.nn.BatchNorm2d(64)
            '''

            if self.feat_type == "double": self.feat_embed1 = nn.Linear(16384*2, 16384)
            else: self.feat_embed1 = nn.Linear(16384, 4096)
            self.batch_norm1 = torch.nn.BatchNorm1d(4096)
            
            self.feat_embed2 = nn.Linear(4096, 256)
            self.batch_norm2 = torch.nn.BatchNorm1d(256)

            self.act = torch.nn.ReLU()

        else:
            self.conv = torch.nn.Conv2d(256, 16, 1, 1, 0) # 64 x 128
    ...
    def get_embed_features(self, x):
        ...
        if not self.ver2:
            if self.feat_type == "single":
                ...
                #x = F.adaptive_avg_pool2d(x, (1, 1)).squeeze(-1).squeeze(-1) # (..., 1, 1)->(...)
            elif self.feat_type == "double":
                x_mean = F.adaptive_avg_pool2d(x, (1, 1)).squeeze(-1).squeeze(-1) # (..., 1, 1)->(...)
                x_max  = F.max_pool2d(x, kernel_size=x.size()[2:]).squeeze(-1).squeeze(-1)
                x = torch.cat([x_mean, x_max], dim=1)

            x = torch.flatten(x, 1)
            x = self.feat_embed1(x)
            x = self.batch_norm1(x)
            x = self.act(x)

            x = self.feat_embed2(x)
    ...
    def forward(self, z1, z2):
        ...
        z1 = self.get_embed_features(z1)
        z2 = self.get_embed_features(z2)
...
class get_feats(nn.Module):
    def __init__(self):
        super().__init__()
        self.get_embed_size = torch.nn.Linear(89, 768)
        self.get_embed_norm = torch.nn.BatchNorm1d(89)

    def forward(self, z):
        z = self.get_embed_norm(z)
        z = self.get_embed_size(z)
        return z
    ...
    def __init__(self,
        ...
        super(CMKD_LIDAR,
              self).__init__(pts_voxel_layer, pts_voxel_encoder,
                             ...
                             train_cfg, test_cfg, pretrained, guided_weights, init_cfg)
...
class CT_BEVDepth(BEVDepth):
    def __init__(self, img_view_transformer=None, 
                    ...
                    pts_neck=None, 
                    **kwargs):
        super(CT_BEVDepth, self).__init__(**kwargs)
