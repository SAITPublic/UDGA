...
from .view_transformer import ViewTransformerLiftSplatShoot_SSL
...
class LSSViewTransformer(BaseModule):
    ...
    def get_lidar_coor(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans,
        ...
        # # points.shape = 1, 6, 59, 32, 44, 3, 1
        # if src2tar is not None: # compensate target image feature, K_t(s2t(K(points)))
        #     s2i = src2tar[:,:,:3,:3].matmul(cam2imgs)
        #     comp_points = s2i.view(B, N, 1, 1, 1, 3, 3).matmul(points).squeeze(-1)
        #     comp_points += src2tar[:,:,:3,3].view(B, N, 1, 1, 1, 3)
        #     K_t = (torch.inverse(cam2imgs)).view(B, N, 1, 1, 1, 3, 3)
        #     points = K_t.matmul(comp_points.unsqueeze(-1))
    ...
    def view_transform_core(self, input, depth, tran_feat):
        ...
        if self.accelerate:
            ...
            # if input[1].shape[3] == 3 : # lyft
            #     coor = self.get_geometry(*input[1:6])
            # else : 
            coor = self.get_lidar_coor(*input[1:7])            
            depth = depth.view(B, N, self.D, H, W)
            tran_feat = tran_feat.view(B, N, self.out_channels, H, W)            
            
            if self.adaptation is not None and N > 6 : # NEEDTO parameterize
                i = N//2
                bev_feat = self.voxel_pooling_v2(
                    coor[:, :i], depth[:, :i], tran_feat[:, :i])
                # with torch.no_grad():
                bev_feat2 = self.voxel_pooling_v2(
                    coor[:, i:], depth[:, i:], tran_feat[:, i:])   
                return (bev_feat, bev_feat2)            
            
            bev_feat = self.voxel_pooling_v2(
                coor, depth.view(B, N, self.D, H, W), 
                tran_feat.view(B, N, self.out_channels, H, W))
            
        # EASY_240308, need to check
...
@NECKS.register_module()
class ViewTransformerLiftSplatShoot_SSL_Carla(ViewTransformerLiftSplatShoot_SSL):    
    # 240502, carla format
    def get_geometry(self, sensor2ego, ego2global, cam2imgs, post_rots, post_trans, bda=None):        
        B, N, _, _ = sensor2ego.shape
        
        c = self.pixel_const
        new_frustum = self.frustum
        new_frustum = new_frustum.unsqueeze(0).unsqueeze(0).repeat(B, N, 1, 1, 1, 1)
        new_intrins = torch.matmul(post_rots, cam2imgs)
        new_frustum[..., 2] = new_frustum[..., 2] / (c* torch.sqrt(1/torch.square(new_intrins[..., 0, 0]) \
                            + 1/torch.square(new_intrins[..., 1, 1]))[..., None, None, None])
                            
        points = new_frustum.to(sensor2ego) - post_trans.view(B, N, 1, 1, 1, 3)
        points = torch.inverse(post_rots).view(B, N, 1, 1, 1, 3, 3)\
            .matmul(points.unsqueeze(-1))
