...
@HEADS.register_module()
class PEFT_SeparateHead(BaseModule):
    """SeparateHead for CenterHead.
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(PEFT_SeparateHead, self).__init__(init_cfg=init_cfg)
        self.heads = heads
        ...
        for head in self.heads:
            ...
            scale1, shift1 = init_ssf_scale_shift(head_conv)
            self.ssfs1.update({head:[scale1, shift1]})
            
            conv_proj = build_conv_layer(
                    conv_cfg,
                    head_conv,
                    classes,
                    kernel_size=final_kernel,
            ...
            self.__setattr__(head+'_proj', conv_proj)
            
            scale2, shift2 = init_ssf_scale_shift(classes)
            self.ssfs2.update({head:[scale2, shift2]})
    ...
    def forward(self, x):
        ...
        ret_dict = dict()
        for head in self.heads:
            head_feat = self.__getattr__(head)(x)
            head_feat = ssf_ada(head_feat, self.ssfs1[head][0], self.ssfs1[head][1])
            head_feat = self.__getattr__(head+'_proj')(head_feat)
            ret_dict[head] = ssf_ada(head_feat, self.ssfs2[head][0], self.ssfs2[head][1])
        return ret_dict


@HEADS.register_module()
class Adapter_SeparateHead(BaseModule):
    """SeparateHead for CenterHead.
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(Adapter_SeparateHead, self).__init__(init_cfg=init_cfg)
        self.heads = heads
        ...
        for head in self.heads:
            ...
            for i in range(num_conv - 1):
                ...
                adapt_layers.append(Adapter(c_in, head_conv, 2))
                c_in = head_conv
            ...
            conv_layers.append(
                build_conv_layer(
                    ...
                    padding=final_kernel // 2,
                    bias=True))
            adapt_layers.append(Adapter(head_conv, classes, 2))

            conv_layers = nn.Sequential(*conv_layers)
            adapt_layers = nn.Sequential(*adapt_layers).cuda()
            
            for param in conv_layers.parameters():
                param.requires_grad = False

            self.__setattr__(head, conv_layers)
            self.adapter[head] = adapt_layers
    ...
    def forward(self, x):
        ...
        for head in self.heads:
            ...
            apt = self.adapter[head](x)
            ret_dict[head] = output + apt
        return ret_dict
        ...
        for idx, task_head in enumerate(self.task_heads):
            ...
            for k in range(num_objs):
                ...
                if width > 0 and length > 0:
                    ...
                    try:vx, vy = task_boxes[idx][k][7:]
                    except: vx, vy = torch.zeros([2], device=task_boxes[idx][k].device)
...
@HEADS.register_module()
class PEFTCenterHead(BaseModule):
    """CenterHead for CenterPoint.
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(PEFTCenterHead, self).__init__(init_cfg=init_cfg)
        ...
        self.scale, self.shift = init_ssf_scale_shift(share_conv_channel)
...
@HEADS.register_module()
class AdapterCenterHead(BaseModule):
    """CenterHead for CenterPoint.
    ...
    def __init__(self,
        ...
        assert init_cfg is None, 'To prevent abnormal initialization ' \
            'behavior, init_cfg is not allowed to be set'
        super(AdapterCenterHead, self).__init__(init_cfg=init_cfg)
        ...
        self.adapter_conv = Adapter(in_channels, share_conv_channel)
