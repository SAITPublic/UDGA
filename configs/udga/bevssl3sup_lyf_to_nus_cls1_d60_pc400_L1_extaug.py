...
_base_ = ['../_base_/datasets/lyft-3d.py',
...
grid_config={
        ...
        'zbound': [-5.0, 3.0, 8.0],
        'dbound': [1.0, 90.0, 1.0],} # Lyft
        # 'dbound': [2.0, 90.0, 1.0],} # nuScenes
        # 'dbound': [1.0, 60.0, 1.0],} # Waymo
...
model = dict(
    type='BEVSSL3_unsup',
    pixel_const=pixel_const,
    # dbg_rgb=True,
    # dbg_str=True,
    ssl_loss='l1_loss',
    ssl_weight=1.0,
    img_backbone=dict(
        pretrained='ckpts/resnet50.pth',
        type='ResNet',
        depth=50,
        num_stages=4,
        out_indices=(2, 3),
    ...
    img_neck=dict(
        ...
        start_level=0,
        out_ids=[0]),
    img_view_transformer=dict(type='ViewTransformerLiftSplatShoot_SSL',
                              pixel_const=pixel_const,
...
dataset_type = 'LyftYawAug'
data_root = 'data/lyft/'
test_dataset_type = 'NuScenesDataset'
test_data_root = 'data/nuscenes/'
data = dict(
    samples_per_gpu=8,
    workers_per_gpu=8,
    train=dict(
    ...
    val=dict(
        ...
        ann_file=test_data_root + 'nuscenes_infos_val.pkl',
...
evaluation = dict(interval=1, start=8, pipeline=test_pipeline, save_best='mAP')
